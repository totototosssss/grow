    ...
    const ITEMS = {
        'energy_drink_law': {
            ...
            use: (gameState, logHelper) => {
                gameState.energy += 25; logHelper.add(`体力が${formatChange(25)}。`);
                gameState.focus += 18; logHelper.add(`集中力が${formatChange(18)}。`);
        ...
        setTimeout(() => {
            ...
            if (internalScore >= passThreshold) {
                if (gameState.luck > 75 && gameState.mental > 80 && internalScore > passThreshold * 1.1) {
                    ...
                    passed = true;
                } else if (Math.random() < (0.20 + Math.max(0,(gameState.knowledge - 70)) / 100 +Math.max(0,(gameState.luck - 70)) / 150 + Math.max(0,(gameState.mental - 70))/200) ) {
                    passed = true;
                }
            } else {
